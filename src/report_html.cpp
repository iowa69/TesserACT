...
const char* kScript =
    "(function(){"
    "var R=document.documentElement,K='tesseract-theme';"
...
std::string sectionRun(const AssemblyReport& rep, const Verdict& v) {
    std::string s = sectionOpen("run", "SECTION 1", "The run and its verdict",
        "TesserACT assembles short reads by building a de Bruijn graph over a ladder of k-mer "
    ...
    exec.push_back({"Tool", "tesseract-asm " + htmlEscape(rep.version)});
...
std::string sectionLadder(const AssemblyReport& rep) {
    std::string s = sectionOpen("ladder", "SECTION 5", "The multi-k ladder",
        ...
        "apart wherever coverage dips or an error survives. TesserACT therefore runs the whole "
...
std::string buildDocument(const AssemblyReport& rep) {
    ...
    s += sectionConfig(rep);
    s += "<footer>Generated by tesseract-asm " + htmlEscape(rep.version) +
         " &middot; Giovanni Lorenzin, IOWA-BioTech"
