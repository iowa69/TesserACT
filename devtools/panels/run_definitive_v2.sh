...
| xargs -P "$JOBS" -n 1 -d '\n' -I{} bash -c '
    set -- $(printf "%s" "{}")
    ./eval_isolate_v3.sh '"$org"' "$1" "$2" "eval/'"$org"'_final/$1" \
        "models/'"$org"'_v2/small_lco.tsm:small" \
        "models/'"$org"'_v2/clonal_lco.tsm:clonal" \
        "models/'"$org"'_v2/clonal_lco.tsm:clonal_f10:TESSERACT_MODEL_MIN_FRACTION=0.1" \
        || echo "  SKIPPED $1"
  '
